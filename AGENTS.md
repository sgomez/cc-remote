...
When working in a git worktree (as code-author agents do):

- **`pnpm lint`** resolves to the primary checkout's `webapp/` directory, not the
  worktree's. Run `npx biome check` from the worktree's `webapp/` directory instead.
- **`pnpm check`** (`tsc --noEmit`) fails in a fresh worktree because
  `routeTree.gen.ts` is generated by the Vite plugin at dev or build time and is
  not committed. The test and lint gates are the reliable pre-commit checks.
...
`setup.sh` itself runs `config.js` inside a throwaway `node:22-slim` container (mounting the repo at `/app`) so the wizard doesn't require Node on the host; it then writes `config.json` (schema-ish record of choices) and compiles `.env` from it, and offers to `docker compose up -d --build` at the end. Phase 1 (the wizard) asks for **infrastructure only** (Caddy toggle, domain, ports) and derives the rest (auth secret, PUID/PGID, agent resource caps). GitHub identity (OAuth credentials, App ID, private key, allow-list) is configured in Phase 2 through the browser bootstrap screen and stored in the Bootstrap File on the data volume. The wizard has no repo, session, or host-path questions: Sessions name themselves and pick their repo in the web UI.
...
### Commit Identity is per-Session, never deployment-wide

`GIT_USER_NAME`/`GIT_USER_EMAIL` are **not** deployment configuration. They are the Commit Identity of the signed-in user who provisioned the Session (`src/core/domain/commit-identity.ts`), resolved in `getCommitIdentity` and carried through `buildSessionEnv` into that Session's container only. The Docker adapter's `infraEnv` contributes none, and neither do the clone helper, the Login Container, or the seed/probe helpers, because none of them commit.

The email is **always** `<id>+<login>@users.noreply.github.com`, never the account's real address. GitHub links a commit to a profile only for an address it recognises for that account, and with "Block command line pushes that expose my email" enabled it rejects (GH007) a push whose commits carry a private address. The legacy `<login>@users.noreply.github.com` form only works for accounts predating the id-qualified one, so it is wrong for anyone signing up today.

This replaced a host-derived deployment-wide value that never actually resolved: `config.js` read `git config --global` inside a container that never mounts `~/.gitconfig`, so every deployment got the `Claude Remote Agent <agent@example.com>` fallback and every agent commit was attributed to nobody. Do not add a fallback back: it could only ever apply when the real identity is missing, which is when creating the Session must fail instead. See `docs/adr/0002-per-user-commit-identity.md`.

The GitHub numeric id needs no persisted field of its own: better-auth writes it to `account.accountId` from the provider profile on every sign-in, and `user.name` is its default `profile.name || profile.login` mapping.

Upgrading a live deployment: Sessions created before this change carry the old environment and keep committing as `Claude Remote Agent`, because container env is fixed at create time. `reset` fixes it but deletes the workspace volume. To repair one in place: `docker exec -it cc-remote-session-<name> git config --global user.email "<id>+<login>@users.noreply.github.com"` (and `user.name`).
...
### Permission Mode is per-Session, and must NEVER be written to shared per-Account state

A Session picks its Permission Mode when it is created (`auto` or `bypassPermissions` — the two the domain allows, see `src/core/domain/permission-mode.ts`), carries it for the container's life in its own `PERMISSION_MODE` env var, and keeps it across a `reset` by recovering it from the `cc-remote-permission-mode` container label. The deployment-wide value the create form starts from is a **Deployment Setting** in the `setting` table, edited on the Settings page.

**Do not restore `permissions.defaultMode` to the seeded `~/.claude.json`.** It looks like an oversight in `entrypoint.sh` and in `wizardSkipConfig`, and it is not: that file lives on the **Account Config Volume**, which every Session of the Account shares. Written there, the mode becomes a shared mutable cell that the last container to start decides for all its siblings — so a Session the operator created as `auto` can end up running `bypassPermissions` because an unrelated sibling started later. It is not hypothetical: when Claude exits, the fallback shell prints a hint to relaunch it, and a relaunch without an explicit flag reads that shared file. Both `agent-session.sh` invocations and that hint pass `--permission-mode` explicitly from the container's own environment; keep it that way.

The `setting` row is the **only** source of the deployment default. `PERMISSION_MODE` is deliberately absent from web-manager's compose environment, from `.env.example`, and from what `config.js` compiles — adding it back would create a second apparent source that silently does nothing.

Upgrading a live deployment: a deployment that set `PERMISSION_MODE` by hand in its `.env` reverts to the filtered default until the operator sets it on the Settings page. Sessions created before this change keep the mode frozen in their container environment and are unaffected until they are `reset`, at which point they adopt the deployment default because they carry no `cc-remote-permission-mode` label.

### Auto Mode

Agent containers default to `claude --permission-mode auto`, which relies on Claude's background safety classifier rather than interactive prompts (there's no TTY approval loop available remotely). Container-level isolation (mounts limited to the session's own workspace + account config volumes, `no-new-privileges`, the two-network split, and the resource limits below) is what makes running unattended in that mode acceptable. See docs/security.md for the full rationale before changing defaults here. A Session may instead be created in `bypassPermissions`, which is the same trade made deliberately per Session rather than once for the machine.
