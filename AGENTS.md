...
Auth is **better-auth v1.6** (`webapp/src/adapters/auth/auth.ts`): GitHub social login (`scope: ["user:email"]`) served from the TSS catch-all `src/routes/api/auth/$.ts`; the public GitHub OAuth callback is `/api/auth/callback/github`. GitHub Apps ignore authorisation-URL scopes — permissions come from the App's own configuration — so `repo` is intentionally absent. Access is gated by `ALLOWED_GITHUB_USERS` and **fails closed**: an empty allow-list denies everyone. The allow-list is keyed on the GitHub `login` (captured via `mapProfileToUser` into a `githubLogin` user field) and enforced in a `databaseHooks.session.create.before` hook that runs on **every** sign-in (returning-user included, so removing someone from the list locks them out), with a `user.create.before` belt for the first sign-up. better-auth persists users, sessions and the GitHub access token in **its own tables on the same SQLite file** as the MikroORM domain tables (WAL mode, two connections); sessions therefore survive a container restart. `BETTER_AUTH_SECRET` must be stable in `.env` or every restart invalidates all sessions. `@better-auth/cli generate|migrate` owns better-auth's schema (`auth:*` scripts); MikroORM migrations own the domain tables (`db:migrate`); `pnpm migrate` runs both, applied idempotently by the container entrypoint on start.

The GitHub access token is retrieved server-side (`auth.api.getAccessToken`) and injected as a one-shot `GITHUB_TOKEN` only into **clone helpers** — ephemeral containers that clone the repo and exit. Session containers no longer carry a durable `GITHUB_TOKEN`; they receive `CC_BROKER_SECRET` (a per-Session secret) and `CC_BROKER_URL` (the credential broker address on the agents network) and fetch installation tokens on demand via a git credential helper. The token itself never reaches the browser.
