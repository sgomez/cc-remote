...
export function makeRegisterAccount(deps: RegisterAccountDeps) {
  ...
  return async function registerAccount(input: RegisterAccountInput): Promise<Account> {
    ...
    if (type.singleton) {
      const existing = await deps.accounts.findAll();
      if (existing.some((a) => a.providerType === type.id)) {
        throw new SingletonAccountExistsError(type.id);
      }
    }
    ...
    if (ownsConfigVolume(type)) {
      const volume = accountConfigVolumeName(account.id);
      await deps.engine.createVolume(volume);
      // host-mount aside, every seeding method starts from the wizard-skip
      // config; oauth then completes login in a Login Container (#14).
